...
from odoo.addons.l10n_pt_future.utils import hashing as pt_hash_utils
from odoo.addons.l10n_pt_future.models.l10n_pt_at_series import AT_SERIES_ACCOUNTING_DOCUMENT_TYPES
...
class AccountMove(models.Model):
    ...
    def _get_invoice_legal_documents(self, filetype, allow_fallback=False):
        """
        EXTENDS account. For Portugal, we store the binary of the invoice's original and reprint.
        If they exist, the binary will be used to render the file. Else, generate the file.
        """
        if filetype == 'pdf' and self.country_code == 'PT':
            ...
            content, report_type = (self.env['ir.actions.report']
                                    .with_context(allow_multiple_companies=True).with_company(self.company_id)
                                    ._pre_render_qweb_pdf('account.report_invoice_with_payments', self.ids))
            content_by_id = self.env['ir.actions.report']._get_splitted_report('account.report_invoice_with_payments', content, report_type)
    ...
    def _post(self, soft=True):
        ...
        # EXTENDS 'account'
        self.filtered(
            lambda move: move.country_code == 'PT' and move.reversed_entry_id
        )._check_reversal_amounts_and_quantities()
