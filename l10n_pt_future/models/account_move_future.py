...
class AccountMove(models.Model):
    ...
    def _check_reversal_amounts_and_quantities(self):
        """
        ...
        for move in self:
            ...
            if (
                len(
                    set(
                        original_move.mapped("currency_id")
                        + reversals.mapped("currency_id")
                    )
                )
                != 1
            ):
            ...
            for line in original_move.invoice_line_ids.filtered(
                lambda l: l.display_type == "product"
            ):
                original_quantities[line.product_id] += (
                    line.product_uom_id._compute_quantity(
                        line.quantity, line.product_id.uom_id
                    )
                )
            for line in reversals.invoice_line_ids.filtered(
                lambda l: l.display_type == "product"
            ):
                reverse_quantities[line.product_id] += (
                    line.product_uom_id._compute_quantity(
                        line.quantity, line.product_id.uom_id
                    )
                )
            ...
            for product, quantity in reverse_quantities.items():
                if product not in original_quantities:
                    exceeding_quantities.append(
                        _("'%s' is not present on the original invoice.", product.name)
                    )
            ...
            credit_note_total = abs(
                sum(move.amount_total_in_currency_signed for move in reversals)
            )
            excess = abs(credit_note_total) - abs(
                original_move.amount_total_in_currency_signed
            )
                ...
                if len(reversals) > 1:
                    raise UserError(
                        _(
                            ...
                            original_invoice=original_move.name,
                            total_value=abs(move.amount_total_in_currency_signed)
                            - excess,
                        )
                    )
                raise UserError(
